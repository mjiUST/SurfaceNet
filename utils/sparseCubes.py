...
def append_dense_2sparseList(prediction_sub, rgb_sub, param_sub, viewPair_sub = None, min_prob = 0.5, rayPool_thresh = 0, \
    ...
    if not (cube_ijk_np == 'N/A'):      # during training process, visualized random cubes don't store this information. OR use `not (cube_ijk_np is 0)` rather than `not (cube_ijk_np == 0)`
