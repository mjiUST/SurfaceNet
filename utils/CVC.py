...
def clusterFromSparseAdjacency(sparseAdjacencyMatrix, N_pts, indexes = [0], indexesMask = []):
    """
    cluster from sparse adjacency matrix
    recursive function

    -----------
    inputs:
        sparseAdjacencyMatrix: coords pairs of sparse matrix, in ascending order WITHOUT main diagonal
        N_pts: how many pts = size along an axis of the sparse matrix
        indexes: in the 1st loop, by default is [0]; in the following loops, list the adjacent pts indexes.
        indexesMask: mask the indexes that has been processed.
    ...
    -----------
    examples:
    >>> sparseAdjacencyMatrix = np.array([[0,0,1,2], [2,5,3,5]]).T    # ascending order WITHOUT main diagonal
    >>> clusterFromSparseAdjacency(sparseAdjacencyMatrix, N_pts = 6)
    ([0, 2, 5, 1, 3, 4], [0, 3, 5])
    >>> sparseAdjacencyMatrix = np.array([[0], [1]]).T
    >>> clusterFromSparseAdjacency(sparseAdjacencyMatrix, N_pts = 4)
    ([0, 1, 2, 3], [0, 2, 3])
    """

    # If don't deepcopy the inputs, the recursive functions may not work as expected owding to the variable reference!
    sparseAdjacencyMatrix = copy.deepcopy(sparseAdjacencyMatrix)
    indexes = copy.deepcopy(indexes)
    indexesMask = copy.deepcopy(indexesMask)
    ...
    for _index in indexes: 
        ...
        if indexes[0] is 0:
            cluster_1stIndex.append(len(indexesMask))
        # sparseAdjacencyMatrix must be in ascending order WITHOUT main diagonal 
        _adjacentIndexes = sparseAdjacencyMatrix[sparseAdjacencyMatrix[:,0] == _index, 1]    # the adjacent pts' indexes stored in the sparseAdjacencyMatrix.
        indexesMask.append(_index)
        N_adj_pts = _adjacentIndexes.shape[0]   # (N_adj_pts, )
        if N_adj_pts is 0:   # check how many adjacent pts
            continue
        # only pass the left sparseAdjacencyMatrix in order to speed up.
        indexesMask, _ = clusterFromSparseAdjacency(sparseAdjacencyMatrix[N_adj_pts:], N_pts, indexes = _adjacentIndexes, indexesMask = indexesMask)
    return indexesMask, cluster_1stIndex if indexes[0] is 0 else None
